...
TSN Configurations:
    ...
    dropout_ratio:      {}
        ...
        if is_flow:
            ...
            self.base_model["Flow"] = self._construct_flow_model(
                self.base_model["Flow"]
            )
            print("Done. Flow model ready...")
        if is_diff:
            print("Converting the ImageNet model to RGB+Diff init model")
            self.base_model["RGBDiff"] = self._construct_diff_model(
                self.base_model["RGBDiff"]
            )
            print("Done. RGBDiff model ready.")
        if is_spec:
            print("Converting the ImageNet model to a spectrogram init model")
            self.base_model["Spec"] = self._construct_spec_model(
    ...
    def _prepare_base_model(self, base_model):
        ...
        if base_model == "BNInception":
            ...
            for m in self.modality:
                self.base_model[m] = getattr(tf_model_zoo, base_model)(model_path=model_path)
    ...
    def freeze_fn(self, freeze_mode):
        ...
        if freeze_mode == "modalities":
            ...
            raise ValueError(
                "Unknown mode for freezing the model: {}".format(freeze_mode)
            )
    ...
    def forward(self, input):
        ...
        for m in self.modality:
            ...
            base_model = getattr(self, m.lower())
            base_out = base_model(
                input[m].view((-1, sample_len) + input[m].size()[-2:])
            )
    ...
    def _get_diff(self, input, keep_rgb=False):
        ...
        input_view = input.view(
            (-1, self.num_segments, self.new_length["RGBDiff"] + 1, input_c,)
            + input.size()[2:]
        )
        if keep_rgb:
    ...
    def _construct_flow_model(self, base_model):
        ...
        new_kernel_size = (
            kernel_size[:1] + (2 * self.new_length["Flow"],) + kernel_size[2:]
        )
        new_kernels = (
            params[0]
            .detach()
    ...
    def _construct_spec_model(self, base_model):
        ...
        new_avg_pooling = nn.AvgPool2d(
            8, stride=pool_layer.stride, padding=pool_layer.padding
        )
    ...
    def _construct_diff_model(self, base_model, keep_rgb=False):
        ...
        if not keep_rgb:
            new_kernel_size = (
                kernel_size[:1] + (3 * self.new_length["RGBDiff"],) + kernel_size[2:]
            )
            new_kernels = (
    ...
    def get_augmentation(self):
        ...
        if "RGB" in self.modality:
            augmentation["RGB"] = torchvision.transforms.Compose(
                [
                    GroupMultiScaleCrop(self.input_size["RGB"], [1, 0.875, 0.75, 0.66]),
        ...
        if "Flow" in self.modality:
            augmentation["Flow"] = torchvision.transforms.Compose(
                [
                    GroupMultiScaleCrop(self.input_size["Flow"], [1, 0.875, 0.75]),
        ...
        if "RGBDiff" in self.modality:
            augmentation["RGBDiff"] = torchvision.transforms.Compose(
                [
                    GroupMultiScaleCrop(self.input_size["RGBDiff"], [1, 0.875, 0.75]),
                    GroupRandomHorizontalFlip(is_flow=False),
                ]
            )
